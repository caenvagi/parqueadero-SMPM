...
include '../../conexion/conexion.php';
            ...
            // if($_REQUEST['inlineRadioOptions'] == "INGRESO"){
                ...
                if(isset($_POST['register'])){
                    ...
                    if(strlen($_POST['fecha_movimiento']) >= 1  && strlen($_POST['movimiento']) && strlen($_POST['desc_movimiento']) && strlen($_POST['valor'])  && strlen($_POST['user_login'])){
                        ...
                        $tipo = trim($_POST['inlineRadioOptions']);
                        
                        
                        $consulta = "   INSERT INTO caja(fecha_movimiento, movimiento, desc_movimiento, valor_ingreso, valor_egreso, user_login, liquidado, caja_tipo) 
                                        VALUES ('$fecha','$movimiento','$desc_movimiento','$valor_ingreso','$valor_egreso','$user_login','$liquidado','$tipo')";
                        $resultado = mysqli_query($mysqli,$consulta);
                        
                        if($resultado){
                            header('location:../caja_movimientos.php?mensaje=guardado')
                            ?>
